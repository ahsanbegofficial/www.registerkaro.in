...
function generateSiteMap(posts) {
  ...
  const sitemap = posts
    .map(e => `<url><loc>${GLOBAL_VARIABLE}/post/${e.slug}</loc><lastmod>${formatDate(e.modified_gmt)}</lastmod></url>`)
    .join('');
...
export async function getServerSideProps({ res }) {
  ...
  while (page <= totalPages) {
    const response = await fetch(`${NEWGLOBAL_VARIABLE}/wordpress/wp-json/wp/v2/posts?page=${page}`);
